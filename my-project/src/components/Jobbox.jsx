...
const jobbox = ({ jobs }) => {
  ...
  const showDetail = (title, emp, country, city, type, publisher, currency, job_max_salary, link, description) => {
    ...
    console.log(title, emp, country, city, type, publisher, currency, job_max_salary, link, description);
  }
  return (
    <>
      <div className='p-5 mt-5 '>
        {
          jobs?.data?.map(i => <div key={i.job_id} className=' shadow-2xl shadow-[#ea7d75] border mt-5 border2 border-black p-5'>
            ...
            <button onClick={() => showDetail(i.job_title, i.employer_name, i.job_country, i.job_city, i.job_employment_type, i.job_publisher, i.job_salary_currency, i.job_max_salary, i.job_apply_link, i.job_description)} className="text-white mt-5  rounded-es-2xl] hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800  rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 bg-[#ea7d75] p-5 ml-5 font-semibold rounded-es-2xl">
              Details
            </button>
          </div>)
        }
